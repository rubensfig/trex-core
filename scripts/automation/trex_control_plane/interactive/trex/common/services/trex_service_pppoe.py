...
class ServicePPPOE(Service):
    ...
    def __init__ (self, mac, verbose_level = Service.ERROR, s_tag=None, c_tag=None):
        ...
        #IP Address
        self.ip = IPv4Address('192.151.10.1').packed
        ...
        while True:
            ...
            # REQUEST state
                ...
                padr = Ether(src=self.get_mac(),dst=self.ac_mac)/ \
                       ...
                       PPPoED(version=1,type=1,code=PPPOEParser.PADR,sessionid=0,len=45)/PPPoED_Tags(_pkt=self.tags)
                ...
                print("PPPOE: {0} <--- PADS from AC '{1}' session_id: '{2}'".format(self.mac, bytes2mac(service.srcmac), self.session_id))
                self.state = 'LCP'
